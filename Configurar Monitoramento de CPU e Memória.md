# Configurar Monitoramento de CPU e Memória

Guia completo para configurar monitoramento de CPU e Memória no Linux com alertas por e-mail em HTML quando o uso ultrapassar 80%.

**Tempo estimado:** ~15 minutos  
**Pré-requisitos:** 
- Postfix configurado com SMTP2GO (ou outro relay SMTP funcional)
- Script `send_html_alert.sh` já configurado (veja o guia de instalação do ClamAV)
...
O `sysstat` inclui o `mpstat` para medição precisa de CPU.
...
**Substitua `seu@email.com` pelo seu e-mail real.** Para múltiplos destinatários, use vírgula: `email1@gmail.com,email2@gmail.com`
...
Para forçar um teste (mesmo sem CPU alta), edite temporariamente o `CPU_THRESHOLD=0`.
...
Para forçar um teste, edite temporariamente o `MEM_THRESHOLD=0`.
...
- **Evita spam de alertas** - Se a CPU subir momentaneamente, você não receberá dezenas de e-mails
- **Load average já é uma média** - Verificar a cada minuto seria redundante
- **Balanceia resposta vs. ruído** - 5 minutos é tempo suficiente para detectar problemas reais sem alertas falsos

Para ambientes críticos que precisam de resposta mais rápida, altere `*/5` para `*/2` (a cada 2 minutos) ou `*/1` (a cada minuto).

---

Pronto! O monitoramento de CPU e Memória está configurado e enviará alertas profissionais em HTML sempre que ultrapassar 80% de uso.
